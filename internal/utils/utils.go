...
func SplitSnippetSlice(snippetSlice *models.Snippets, batchSize uint) ([]models.Snippets, error) {
	if batchSize == 0 {
	...
	res := make([]models.Snippets, (len(*snippetSlice)-1)/int(batchSize)+1)
	...
	lastBatch := make(models.Snippets, 0, batchSize)
...
func ReverseSnippetMap(snippetMap *models.SnippetMap) map[*models.Snippet]uint64 {
	res := make(map[*models.Snippet]uint64, len(*snippetMap))
...
func FilterSnippetSlice(snippetSlice *models.Snippets, filter ...*models.Snippet) models.Snippets {
	if len(filter) == 0 {
		return *snippetSlice
	}
	...
	res := make(models.Snippets, 0, len(*snippetSlice))

	filtered := func(snippetPtr *models.Snippet) bool {
...
func SliceToMap(slice *models.Snippets) (models.SnippetMap, error) {
	res := make(models.SnippetMap, len(*slice))
